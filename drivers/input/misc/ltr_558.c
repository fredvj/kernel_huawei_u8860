...
/*New Proximity table*/
#define MIN_PROX   1800
/*sunlight_attenuation means the proximity value will increase under the Strong sun light*/
static int sunlight_attenuation  = 410;
static int friction = 250;
static int ltr_558_pwindows_value = 130;
static int ltr_558_pwave_value = 25;
static int min_proximity_value = MIN_PROX;
...
static int ltr558_als_enable(struct ltr558_data *ltr,int gainrange)
{
    ...
    value_reg = get_ltr_register(ltr, LTR558_ALS_CONTR);
    LTR558_DBG("%s:value_reg = 0x%x\n",__func__,value_reg);
    if (gainrange == 1)
        ...
        ret = -1;
	/*READ because it's possible to cause implement fail*/
    value_reg = get_ltr_register(ltr, LTR558_ALS_CONTR);
    LTR558_DBG("%s:after value_reg = 0x%x\n",__func__,value_reg);
    mdelay(WAKEUP_DELAY);
    return ret;
}
...
static int ltr558_als_read(struct ltr558_data *ltr,int gainrange)
{
    ...
    /*all parameters are magnified by 1000,but ratio is 100 */
    else if(ratio >= 85)
    {
        lux = 0;
        return lux;
    }
    lux = (alsval_ch0*ch0_x - alsval_ch1*ch1_x) / 1000;
...
static void ltr558_work_func(struct work_struct *work)
{
    ...
    int alsval_ch0_lo, alsval_ch0_hi, alsval_ch0;
    int lux = 0;
    static int last_minh = MIN_PROX;
    static int last_minl = MIN_PROX;
    static int min_record = MIN_PROX;
    static int last_cdata = 0;
    /*Print test.txt's debug*/
#if 0
    unsigned int fp ;
    char test[500]={"0"};
#endif
    ...
    if( PS_INIT == (interrupt & PS_INIT) )
    {
        ...
        LTR558_DBG("pdata = %d  and cdata = %d and last_cdata = %d\n",pdata,cdata,last_cdata);  
        LTR558_DBG("ps_contl = 0x%x  \n",get_ltr_register( ltr_data, LTR558_PS_CONTR));
        /*RESET the pdata under the strong sunlight*/
        if(last_cdata > sunlight_attenuation)
        {
            pdata = min_record;
            LTR558_DBG("pdata cut!!\n");
        }
        if ((pdata + ltr_558_pwave_value) < min_proximity_value)
        {
			/*move this line to below*/
            min_proximity_value = pdata + ltr_558_pwave_value;
	        /*it's useless to reset the Vh or Vl at this place*/
            if(min_record > min_proximity_value)
            {
                min_record = min_proximity_value;
            }
            LTR558_DBG("readjust the min_proximity_value to %d \n",min_proximity_value);
        }
        ...
        {
            /* stop the close init */
            if(last_minh + friction == pthreshold_h)
            {
                ...
                temp = min_proximity_value + friction;
        ...
        else if (pdata <=  pthreshold_l)
        {
            ...
            /*15 is higher than pthreshold_l to make sure these is other interrupt to readjust the min_prox value*/
            if( last_minl + 10 == pthreshold_l)
            {
            ...
            {
                temp = min_proximity_value + 10;
                ret  = set_ltr_register(ltr_data,LTR558_PS_THRES_LOW_0,temp & 0xff);
                ret |= set_ltr_register(ltr_data,LTR558_PS_THRES_LOW_1,temp >> 8 );
                last_minl = min_proximity_value;
                min_proximity_value = MIN_PROX;
                LTR558_DBG("min_prox is readjust\n");
            }
        ...
        ret  = set_ltr_register(ltr_data,LTR558_PS_THRES_LOW_0,0);
        ret |= set_ltr_register(ltr_data,LTR558_PS_THRES_LOW_1,0);
        ret |= set_ltr_register(ltr_data,LTR558_PS_THRES_UP_0, 1);
        ret |= set_ltr_register(ltr_data,LTR558_PS_THRES_UP_1, 0); 
    #endif
    }
    if( ALS_INIT == (interrupt & ALS_INIT) ) 
    {
		...
		/*reset the ALS interrupt module*/
        alsval_ch0_lo = get_ltr_register(ltr_data,LTR558_ALS_DATA_CH0_0);
        alsval_ch0_hi = get_ltr_register(ltr_data,LTR558_ALS_DATA_CH0_1);
        alsval_ch0 = (alsval_ch0_hi * 256) + alsval_ch0_lo;
        cdata = lux ;
        cdata_high = alsval_ch0 + 1 ;
        cdata_low =  alsval_ch0 ;
        if( 0 >= cdata_low)
        {
            cdata_low = 1;
            cdata_high=1;
		...
		/*PRINT-LOG on phone*/
	#if 0
        fp =sys_open("/sdcard/test.txt",O_CREAT|O_RDWR,0600);
        sprintf(test,"cdata=%d,lux=%d,padta=%d\n",cdata, lux,pdata);
        sys_write(fp, test,sizeof(test));
        sys_close(fp);
	#endif
        ...
        LTR558_DBG(KERN_ERR "%s:cdata=%d,cdata_high = %d,cdata_low = %d\n",__func__,cdata,cdata_high,cdata_low);
        last_cdata = cdata;
   #if 0    
...
{ 
    ...
    if (client->irq) 
    {
		...
		/*change to 60Khz,75%duty,5-emmit*/
        ret   = set_ltr_register(ltr,LTR558_PS_LED, 0x77);
        ret  |= set_ltr_register(ltr,LTR558_PS_MEAS_RATE, 0x02);
        ret  |= set_ltr_register(ltr,LTR558_PS_N_PULSES, 0x05);
        ret  |= set_ltr_register(ltr,LTR558_PS_CONTR, 0x00);
